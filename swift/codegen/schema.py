...
@group("decl")
class IfConfigClause(Locatable):
...
class IfConfigDecl(Decl):
    clauses: list[IfConfigClause] | child
...
class UnresolvedDeclRefExpr(Expr):
    name: optional[string]

class UnresolvedDotExpr(Expr):
    base: Expr | child
    name: string

@qltest.skip
class UnresolvedMemberExpr(Expr):
    name: string

class UnresolvedPatternExpr(Expr):
    sub_pattern: Pattern | child

@qltest.skip
class UnresolvedSpecializeExpr(Expr):
    pass
...
class UnresolvedMemberChainResultExpr(IdentityExpr):
    pass

class UnresolvedTypeConversionExpr(ImplicitConversionExpr):
    pass
